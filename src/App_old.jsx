...
export default function App() {
  ...
  const [menu, setMenu] = useState(null); // State: { type, step, mfr, model }

  const [loadout, setLoadout] = useState({
    engine: GEAR_DATA.engines[0], // Thor 202 WC
    frame: GEAR_DATA.frames[0],   // Maverick
    glider: { ...GEAR_DATA.manufacturers.Dudek.models["DriftAir 2"][2], brand: 'Dudek', model: 'DriftAir 2' },
    accessory: GEAR_DATA.accessories[0],
    helmet: true, gloves: true, boots: true, reserve: true
  });
  ...
  const stats = useMemo(() => {
    const gearBase = (loadout.engine?.weight || 0) + (loadout.frame?.weight || 0) + (loadout.glider?.weight || 0) + (loadout.accessory?.weight || 0);
    const total = gearBase + pilotWeight + (fuelLiters * 0.748) + 4.5; // Standard parts
  ...
  return (
    <div className="app-viewport">
      ...
      <div className={`side-drawer ${menu ? 'open' : ''}`}>
        ...
        <div className="pr-4 h-[75vh] overflow-y-auto custom-scrollbar">
          {/* Engine Selection */}
      ...
      <div className="character-screen">
        ...
        <div className="flex flex-col gap-10 justify-center items-start pl-12">
          ...
          <Socket label="RESERVE" active={loadout.reserve} sub="Chute" onClick={() => setLoadout({...loadout, reserve: !loadout.reserve})} />
      ...
      {/* MASSIVE 1400PX ATTRIBUTE PEDESTAL */}
      <div className="attribute-box space-y-12">
        ...
        <div className="grid grid-cols-2 gap-16">
          <div>
            <p className="attr-label-huge">Wing Loading</p>
            <p className="attr-value-huge text-blue-400">{stats.loading.toFixed(2)}</p>
          </div>
          <div className="text-right">
            <p className="attr-label-huge">Empty Mass</p>
            <p className="attr-value-huge text-amber-500">{convert(stats.empty).toFixed(1)}</p>
          </div>
        </div>
      </div>
    </div>
